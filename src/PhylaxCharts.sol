...
abstract contract PhylaxCharts is PhylaxBase {
    ...
    modifier uniqueChartName(string memory chartName) {
        require(
            bytes(chartByName[chartName].chartName).length == 0,
    ...
    function initCharts()
        internal
        virtual
        returns (PhylaxCharts.Chart[] memory memCharts)
    {
        memCharts = new PhylaxCharts.Chart[](PhylaxCharts.phylaxCharts.length);

        for (uint256 i; i < phylaxCharts.length; i++) {
            Label[] memory labels = new Label[](
                phylaxCharts[i].labelKeys.length
            );

            for (uint256 j; i < phylaxCharts[i].labelKeys.length; j++) {
                labels[j] = Label({
                    key: phylaxCharts[i].labelKeys[j],
                    value: phylaxCharts[i].labelValues[j]
                });
            }
    ...
    function createMultiChart(
        ...
        extendedLabels[labels.length] = Label({
            key: "multiChartName",
            value: multiChartName
        });
    ...
    function writeToChart(
        string memory chartName,
        uint64 value,
        Label[] memory labels
    ) internal {
        require(
            chartByName[chartName].dataPointType == DataPointType.Uint,
            "Invalid dataPointType"
        );
    ...
    function writeToChart(
        ...
        require(
            chartByName[chartName].dataPointType == DataPointType.Int,
            "Invalid dataPointType"
        );
    ...
    function writeToChart(
        ...
        require(
            chartByName[chartName].dataPointType == DataPointType.String,
            "Invalid dataPointType"
        );
        writeDataPointString(
            chartName,
            vm.toString(value),
            includeEnvInfo(labels)
        );
    }
    ...
    function includeEnvInfo(
        Label[] memory labels
    ) internal view returns (Label[] memory) {
        ...
        newLabels[labels.length] = Label({
            key: "block_number",
            value: vm.toString(block.number)
        });
        newLabels[labels.length + 1] = Label({
            key: "chain_id",
            value: vm.toString(block.chainid)
        });
        return newLabels;
    }
}
